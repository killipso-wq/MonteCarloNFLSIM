...
- Optimizer (to receive new “Build from nfl_data_py” section)
...
- Optimizer integration: “Build from nfl_data_py” section that auto-loads outputs into Optimizer tab
- DST enhancements in adapter path (team defense stats → DST)

--------------------------------------------------------------------------------

19) Notes on the Monte Carlo PDF (methodology alignment)

- Reproducible trials: --sims (trials), --seed; record config in metadata.json
- Distributional summaries: export p10/p50/p75/p90/p95 and exceedance probabilities
- Parameterization: per-player distributions centered on priors or site FPTS (rookies), variance from position/priors
- Adjustments: OU/SPRD/ML/TM/P nudge environment; DK scoring with bonuses; clamp at zero
- Reporting: accuracy vs site (MAE/RMSE/corr), coverage rates, big-delta flags
